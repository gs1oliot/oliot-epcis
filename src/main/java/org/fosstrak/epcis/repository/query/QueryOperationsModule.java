 ...
 * Control Interface. The implementation converts invocations from CXF into CQL
 * queries and returns the results back to the requesting client through CXF.
 * 
 * @author David Gubler
 * @author Alain Remund
...
public class QueryOperationsModule implements EpcisQueryControlInterface {
    ...
    private QueryOperationsBackendCassandra backendCassandra;
            ...
            try {
                if (paramName.equals("eventType")) {
                    ...
                    if (paramName.equals("GE_eventTime")) {
                        aggrEventQuery.setHasGE_Eventtime(true);
                        objEventQuery.setHasGE_Eventtime(true);
                        quantEventQuery.setHasGE_Eventtime(true);
                        transEventQuery.setHasGE_Eventtime(true);
                    }
                    if (paramName.equals("LT_eventTime")) {
                        aggrEventQuery.setHasLT_Eventtime(true);
                        objEventQuery.setHasLT_Eventtime(true);
                        quantEventQuery.setHasLT_Eventtime(true);
                        transEventQuery.setHasLT_Eventtime(true);
                    }
                    ...
                    if (!aos.getString().isEmpty()) {
                        ...
                        if (paramName.equals("EQ_bizLocation")) {
                            aggrEventQuery.setHasLocation(true);
                            quantEventQuery.setHasLocation(true);
                            transEventQuery.setHasLocation(true);
                            objEventQuery.setHasLocation(true);
                        }
                    ...
                    if (!aos.getString().isEmpty()) {
                        ...
                        aggrEventQuery.setHasMatchEpc(true);
                        objEventQuery.setHasMatchEpc(true);
                        transEventQuery.setHasMatchEpc(true);
                    ...
                    ArrayOfString aos = parseAsArrayOfString(paramValue);
                    if (!aos.getString().isEmpty()) {
                        aggrEventQuery.addEventQueryParam("parentID", Operation.EQ, aos.getString());
                        transEventQuery.addEventQueryParam("parentID", Operation.EQ, aos.getString());
                        aggrEventQuery.setHasParentID(true);
                        transEventQuery.setHasParentID(true);
                    }
                    ...
                    ArrayOfString aos = parseAsArrayOfString(paramValue);
                    if (!aos.getString().isEmpty()) {
                        quantEventQuery.addEventQueryParam("epcClass", Operation.EQ, aos.getString());
                        quantEventQuery.setHasMatchEpc(true);
                    }
...
//                    nofEventFieldExtensions++;
                    String eventFieldExtBase = "extension!" + fieldname.replace("#", "_").replace("://", "virgule_").replace(":","collon_").replace(".", "dot_");
...
//        }
    ...
    private Map<String, QuerySubscriptionScheduled> loadSubscriptions(Session session)
            throws ImplementationExceptionResponse, DriverException {
        ...
        if (subscriptions == null) {
            ...
            subscriptions = backendCassandra.fetchSubscriptions(session);
        }
    ...
    public List<String> getSubscriptionIDs(String queryName) throws NoSuchNameExceptionResponse,
        ...
        try {
            ...
            try {
                session = cassandraResource.createOrGetSession();
    ...
    public QueryResults poll(String queryName, QueryParams queryParams) throws NoSuchNameExceptionResponse,
        ...
        try {
            LOG.debug("------------------------------------------------------------------------------");
            LOG.debug("Keyspace: " + cassandraResource.getKeyspace());
            ...
            try {
                ...
                sessionCassandra = cassandraResource.createOrGetSession();
                ...
                if (queryName.equals("SimpleEventQuery")) {
                    ...
                    for (SimpleEventQueryDTO eventQuery : eventQueries) {
                        ...
                        backendCassandra.setAllowFiltering(allowFiltering);
                        backendCassandra.prepareSimpleEventQuery(sessionCassandra, eventQuery, eventList.getObjectEventOrAggregationEventOrQuantityEvent());
                    ...
                    backendCassandra.runMasterDataQuery(sessionCassandra, mdQuery, vocList.getVocabulary());
    ...
    public void subscribe(String queryName, QueryParams params, String dest, SubscriptionControls controls,
        ...
        try {
            ...
            try {
                ...
                if (backendCassandra.fetchExistsSubscriptionId(session, subscriptionID)) {
               ...
               Map<String, QuerySubscriptionScheduled> subscribedMap = loadSubscriptions(session);
                ...
                backendCassandra.storeSupscriptions(session, params, dest, subscriptionID, controls, triggerURI,
...
//                }
        ...
        try {
            ...
            try {
                ...
                if (subscribedMap.containsKey(subscriptionID)) {
                    ...
                    backendCassandra.deleteSubscription(session, subscriptionID);
                } else {
    ...
    public QueryOperationsBackendCassandra getbackendCassandra() {
        return backendCassandra;
    }

    public void setbackendCassandra(QueryOperationsBackendCassandra backendCassandra) {
        this.backendCassandra = backendCassandra;
    }
    ...
    public CassandraResource getCassandraResource() {
        return cassandraResource;
    }

    public void setCassandraResource(CassandraResource cassandraResource) {
