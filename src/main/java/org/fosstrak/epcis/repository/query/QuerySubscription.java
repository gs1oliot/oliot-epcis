...
import org.fosstrak.epcis.repository.ApplicationContextProvider;
...
public class QuerySubscription implements EpcisQueryCallbackInterface, Serializable {
    ...
    public QuerySubscription(final String subscriptionID, final QueryParams queryParams, final String dest,
        ...
        if (subscribtionbyEventTime()) {
            updateEventTime(queryParams, initialRecordTime);
        } else {
    ...
    private void updateSubscriptionCQL(final Calendar lastTimeExecuted) {        
        try {
            CassandraResource cassandraResource = (CassandraResource) ApplicationContextProvider.getApplicationContext().getBean("CassandraBean", CassandraResource.class);
            ...
            PreparedStatement statement= session.prepare(update);
            BoundStatement boundStatement = new BoundStatement(statement);
            
            LOG.debug("SQL: " + update);
            boundStatement.setDate(0, lastTimeExecuted.getTime());
            LOG.debug("       query param 1: " + lastTimeExecuted.getTime());
            ...
            LOG.debug("       query param 2: [" + " bytes]");
            
            boundStatement.setString(2, subscriptionID);
            LOG.debug("       query param 3: " + subscriptionID);
            
            session.execute(boundStatement);

        } catch (DriverException e) {
            String msg = "An CQL error occurred while updating the subscriptions in the database.";
            LOG.error(msg, e);
        } catch (IOException e) {
    ...
    private void updateRecordTime(final QueryParams queryParams, final Calendar initialRecordTime) {
        ...
        if (this.initialRecordTime.compareTo(initialRecordTime) != 0)
            updateSubscriptionCQL(initialRecordTime);
    }
    
    private void updateEventTime(final QueryParams queryParams, final Calendar initialRecordTime) {
        boolean foundEventTime = false;        
        for (QueryParam p : this.queryParams.getParam()) {
            if (p.getName().equalsIgnoreCase("GE_eventTime")) {
                LOG.debug("Updating query parameter 'GE_eventTime' with value '" + initialRecordTime + "'.");
                p.setValue(initialRecordTime);
                foundEventTime = true;
            }
        }
        for (QueryParam p : this.queryParams.getParam()) {
            LOG.debug(p.getName()+" has Value: " + p.getValue().toString());
        }
        if (!foundEventTime) {
            LOG.debug("Adding query parameter 'GE_eventTime' with value '" + initialRecordTime + "'.");
            QueryParam newParam = new QueryParam();
            newParam.setName("GE_eventTime");
            newParam.setValue(initialRecordTime);
            this.queryParams.getParam().add(newParam);
        }
        if (this.initialRecordTime.compareTo(initialRecordTime) != 0)
    ...
    public void executeQuery() {
        ...
        if (subscribtionbyEventTime()) {
            updateEventTime(queryParams, lastTimeExecuted);
        } else {
...
//        EPCISServicePortType servicePort = service.getPort(portName, EPCISServicePortType.class);
    ...
    private boolean subscribtionbyEventTime() {
        Properties properties = getProperties();
        return Boolean.parseBoolean(properties.getProperty("subscription.by.EventTime", "true"));
    }
